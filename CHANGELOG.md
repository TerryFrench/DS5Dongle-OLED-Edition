...
- **(Reverted)** An attempt to declare `0xFA`/`0xFB`/`0xFC` in the HID report descriptor (to make Chrome WebHID accept `receiveFeatureReport()` for them) **broke device discovery entirely** — the modified descriptor stopped the device being visible to WebHID *and* the PlayStation Accessories app (it still enumerated in Device Manager). Reverted `src/usb_descriptors.cpp` to the known-good descriptor (`0x0141`/`0x01B5`, no vendor feature reports declared). The live web-telemetry path will be revisited without touching the DualSense HID report descriptor. The `0xfc` firmware handler and temperature smoothing above are retained (they don't affect USB enumeration; the web read just fails soft to mock values, as before).
