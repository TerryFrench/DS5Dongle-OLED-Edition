...
- **HID report descriptor did not declare `0xFA`/`0xFB`/`0xFC` as feature reports.** Both `desc_hid_report_ds` and `desc_hid_report_dse` in `src/usb_descriptors.cpp` stopped at `0xF9`, so Chrome WebHID rejected `receiveFeatureReport()` for the OLED Edition vendor reports — the web config emulator's **slots, diagnostics, and CPU/Clock screens silently showed mock/default values when a controller was connected** (only input-report screens like the joystick view worked). Added the three Report ID / Usage / Report Count(63) / Feature blocks to both descriptors (`static_assert` sizes updated `0x141→0x159` and `0x1B5→0x1CD`). Pre-existing for `0xFA`/`0xFB` since those reports were introduced; only surfaced now while wiring up live CPU telemetry.
- **Configuration descriptor `wDescriptorLength` was not bumped with the report descriptor** (regression from the line above, same dev cycle). It still advertised 321 (`0x0141`) while the report descriptor was now 345 bytes, so the host fetched a truncated, invalid HID report descriptor: the device still enumerated (Device Manager showed the DualSense fine) but **WebHID and the PlayStation Accessories app could not see it at all**. Updated to 345 (`0x0159`) for DS / 461 (`0x01CD`) for DSE, with a comment tying it to `sizeof(desc_hid_report_ds)` so the two can't silently drift again.
