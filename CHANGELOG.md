...
- **Web telemetry: `0xFA`/`0xFB`/`0xFC` feature reports not readable.** A browser-side read-only diagnostic confirmed the cause definitively: Chrome WebHID returns `NotAllowedError` for any report ID **not declared** in the parsed HID report descriptor (declared `0xF7`/`0xF8`/`0xF9` read fine; undeclared `0xFA`/`0xFB`/`0xFC` all fail). So the OLED Edition vendor reports must be declared. Declared all three in both `desc_hid_report_ds` and `desc_hid_report_dse` (same 8-byte shape as the existing `0xF6`–`0xF9`), with `wDescriptorLength` (DS `0x0159`, DSE `0x01CD`) and the `static_assert`s updated **in the same change** so there is never a truncated-descriptor intermediate. An earlier attempt regressed because (a) `wDescriptorLength` was bumped one commit late, flashing a genuinely-broken intermediate, and (b) **Windows caches the parsed HID report descriptor keyed by VID/PID/`bcdDevice`**, so a corrected reflash kept serving the stale broken descriptor. Fixed both: atomic descriptor change + `bcdDevice` bumped `0x0100 → 0x0101` to force Windows to re-read. Comments at `wDescriptorLength`, the `static_assert`, and `bcdDevice` now cross-reference so this can't drift again.
